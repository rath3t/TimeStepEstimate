...
#include "membranestrain.hh"
#include <ikarus/utils/tensorutils.hh>
#include "directorFunctions.hh"
#include "transverseshear.hh"
...
//    }
    ...
    auto createFunctions(const FERequirementType &par,
      ...
      const auto &dNodal = par.getGlobalSolution(Ikarus::FESolutions::noSolution)[_1];
      ...
      const std::string &directorFunctionType = "NFE";
    ...
    template<typename ScalarType>
    void calculateMatrixImpl(const FERequirementType &par, typename Traits::MatrixType hred,
                             const std::optional<const Eigen::VectorX<ScalarType>> &dx = std::nullopt) const {
      using namespace Dune::Indices;
      hred.setZero();
...
//            Eigen::Matrix<ScalarType, 2, 2> kgShear= kgDirectorDirectorShear(kin,Ndirector,dNdirector,gpIndex,i,j,displacementFunction,directorFunction,S);
    ...
    template<typename ScalarType>
    void calculateVectorImpl(const FERequirementType &par, typename Traits::VectorType rieGrad,
                             const std::optional<const Eigen::VectorX<ScalarType>> &dx = std::nullopt) const {
      using namespace Dune::Indices;
      rieGrad.setZero();
